...
function handler (req, res) {
  fs.readFile(__dirname + '/client20.html',
  function (err, data) {
    if (err) {
      res.writeHead(500);
...
console.log("Server connection function called!");
  ...
  socket.on('setname', function(userName) {  
	console.log("Server setname function called!");  
  ...
  socket.on('syncplayers', function(user) {  
	console.log("Server syncplayers function called!");
    users = user;  
  });

  // Listens for keypress and sends back to client
  socket.on('keypress', function (data) {
	console.log("Server keypress function called!");
    io.sockets.emit('playermove', data[0], data[1]); 
  });  

  // Listens for clients leaving game
  socket.on('leave', function (userName) {
console.log("Server leave function called!");
    var l = users.length;
    for (var i = 0; i < l; i++) {
